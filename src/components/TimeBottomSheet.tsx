...
export function TimeBottomSheet({ open, value, onClose, onConfirm }: TimeBottomSheetProps) {
  ...
  const adjust = (field: "month" | "day" | "hour" | "minute", delta: number) => {
    setDraft((prev) => {
  ...
  const Row = ({
    ...
    <div className="flex items-center justify-between rounded-xl bg-slate-50 px-4 py-3 shadow-[inset_0_1px_3px_rgba(15,23,42,0.05)]">
      ...
      <div className="flex items-center gap-3">
        <button type="button" onClick={onMinus} className="rounded-lg p-2 text-slate-500 hover:bg-slate-200/60">
          <Minus className="h-4 w-4" />
        </button>
        <span className="min-w-[4.5rem] text-center font-semibold tabular-nums text-slate-800">{display}</span>
        <button type="button" onClick={onPlus} className="rounded-lg p-2 text-slate-500 hover:bg-slate-200/60">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      <button
        ...
        aria-label="닫기"
      />
      <div className="relative z-10 w-full max-w-lg rounded-t-2xl bg-white p-6 pb-8 animate-slide-up shadow-[0_-4px_32px_rgba(15,23,42,0.12)]">
        ...
        <p className="mt-4 text-center text-sm text-slate-500">{formatDateLabel(draft)} (퇴근 시간)</p>
        <div className="mt-6 flex gap-3">
          <Button variant="secondary" className="flex-1" onClick={onClose}>
            취소
          </Button>
          <Button className="flex-1" onClick={() => { onConfirm(draft); onClose(); }}>
            적용
          </Button>
        </div>
      </div>
...
export function TimePickerButton({ value, onClick, className }: TimePickerButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
