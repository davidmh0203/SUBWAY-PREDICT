...
import type { HourlyCongestion } from "@/lib/types";
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-base font-bold leading-snug text-slate-900">
            [{lineName} {stationName}역]으로 진입하는 열차의 시간대별 혼잡도
          </h2>
          <p className="mt-1 text-xs text-slate-500">
            {dateRange} ~ {dateRange}, {direction}, {dayNames[today.getDay()]}요일
          </p>
        </div>
        {onDirectionSwap && (
          <button
      ...
      <div className="relative">
        ...
        <div className="ml-4 h-44">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} barCategoryGap={2} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
              <XAxis
                dataKey="hour"
                tickFormatter={(h) => `${h}시`}
              ...
              <Bar dataKey="rate" radius={[2, 2, 0, 0]} maxBarSize={14}>
                {data.map((entry) => (
                  <Cell
                    ...
                    opacity={activeHour !== undefined && entry.hour === activeHour ? 1 : 0.92}
                  />
                ))}
              </Bar>
            </BarChart>
