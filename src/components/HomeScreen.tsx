...
export function HomeScreen({ form, onFormChange, onSearch }: HomeScreenProps) {
  ...
  return (
    <div className="animate-fade-in space-y-5 pb-24">
      ...
      <Card className="bg-amber-50/60 shadow-[inset_0_1px_3px_rgba(245,158,11,0.06),0_2px_12px_rgba(245,158,11,0.08)]">
        <CardContent className="p-4 pt-4">
          ...
          <p className="text-sm leading-relaxed text-slate-700">
            🌧️ 18:00 퇴근길 비 예보 + 🎤 잠실 콘서트(2만명)
          </p>
          <p className="mt-1 text-sm text-rose-700">
            → 2호선 사당-잠실 구간 18시~20시 혼잡도 <strong>140%</strong> 폭증 예상
          </p>
        </CardContent>
      </Card>
      ...
      <section>
        <h2 className="mb-3 flex items-center gap-2 text-sm font-medium text-slate-600">
          <Star className="h-4 w-4 text-amber-500" /> 자주 가는 쾌적 경로
        </h2>
        <Card
        ...
        >
          <CardContent className="p-4">
            <p className="font-medium text-slate-800">🏠 집 ➔ 🏢 회사 (2호선 오피스 라인)</p>
            <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-slate-500">
              <span>소요시간 32분</span>
              <span className="text-slate-300">|</span>
              <span>현재: 🟢 여유</span>
              <Badge variant="warning" className="text-[10px]">30분 뒤 🟡 주의 예상</Badge>
            </div>
          </CardContent>
        </Card>
      </section>
