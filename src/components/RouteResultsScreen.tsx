...
import type { RoutePath, SearchForm } from "@/lib/types";

interface RouteResultsScreenProps {
  form: SearchForm;
  onBack: () => void;
  onSelectRoute: (route: RoutePath) => void;
...
export function RouteResultsScreen({
  ...
  const [direction, setDirection] = useState<"강남 방면" | "신도림 방면">("강남 방면");
  ...
  const routes = useMemo(() => buildRoutes(debouncedTime), [debouncedTime]);
      ...
      <Card>
        <CardContent className="space-y-4 p-4 pt-5">
          <div className="flex items-center justify-between">
            <p className="text-xs text-slate-500">출발 시각을 조절하면 아래 차트가 갱신됩니다</p>
            {loading && <RefreshCw className="h-4 w-4 animate-spin text-slate-400" />}
          </div>

          <div className="flex justify-between px-1 text-xs text-slate-400 tabular-nums">
            {SLIDER_MARKS.map((t, i) => (
              <span key={t} className={i === sliderIndex ? "font-bold text-slate-800" : ""}>
                {t}
              </span>
            ))}
          </div>
          ...
          <HourlyCongestionChart
            ...
            direction={direction}
            onDirectionSwap={() =>
              setDirection((d) => (d === "강남 방면" ? "신도림 방면" : "강남 방면"))
            }
          />
        </CardContent>
      </Card>
      ...
      <section className="space-y-3">
        ...
        {routes.map((route, idx) => {
          ...
          return (
            <Card
              key={route.id}
              className={`cursor-pointer transition-shadow duration-300 hover:shadow-[0_2px_6px_rgba(15,23,42,0.06),0_8px_24px_rgba(15,23,42,0.07)] ${
                route.recommended ? "shadow-[0_2px_8px_rgba(15,23,42,0.08)]" : ""
              }`}
            ...
            >
              <CardContent className="p-4">
                <div className="mb-2 flex flex-wrap items-center gap-2">
                  ...
                  {route.recommended && <Badge variant="smooth" className="ml-auto">추천</Badge>}
                </div>
                <div className="flex flex-wrap gap-3 text-sm text-slate-600">
                  ...
                  <span>{route.lineName} · 환승 {route.transfers}회</span>
                </div>
