...
import type { MetroLineSegment } from "./metro-network";
import { LINE_END_BADGES } from "./metro-line-badges";

export const BASE_STATION_R = 4.5;
export const TRANSFER_STATION_R = BASE_STATION_R * 1.5;
...
/** 역별 수동 보정 (밀집 구간) */
const LABEL_OVERRIDES: Record<string, LabelLayout> = {
...
export function getStationMarkerRadius(meta: StationMeta): number {
  return meta.isTransfer ? TRANSFER_STATION_R : BASE_STATION_R;
}

function markerOverlap(
  cx: number,
  cy: number,
  meta: StationMeta,
...
export function computeAllStationMeta(): Map<string, StationMeta> {
  const map = new Map<string, StationMeta>();

  for (const station of METRO_STATIONS) {
    const endpointColors = new Set<string>();
...
type LabelCandidate = LabelLayout & { ox: number; oy: number };
...
export function computeLabelLayouts(
  ...
  for (const station of sorted) {
    ...
    const stationMeta: StationMeta = m ?? {
      isTransfer: false,
      lineColor: "#64748b",
      lineKeys: [],
      lineColors: [],
...
export const STATION_META = computeAllStationMeta();
export const STATION_LABELS = computeLabelLayouts(STATION_META);

export function getStationMeta(station: MetroStation): StationMeta {
  return (
    STATION_META.get(station.id) ?? {
      isTransfer: false,
      lineColor: "#64748b",
      lineKeys: [],
      lineColors: [],
...
export function getLabelLayout(stationId: string): LabelLayout {
  return (
    STATION_LABELS.get(stationId) ??
    LABEL_OVERRIDES[stationId] ?? {
      x: 0,
      y: 0,
      anchor: "start",
      rotate: -38,
