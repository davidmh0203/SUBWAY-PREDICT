...
export function OdsayApiTest() {
  ...
  // --- 길찾기 경로 테스트 ---
  const [routeStart, setRouteStart] = useState('연신내')
  const [routeEnd, setRouteEnd] = useState('봉은사')
  ...
  const run역SuffixCompare = useCallback(async () => {
    ...
    try {
      ...
      setCompareResult({
        ...
        note:
          '정규화 후 「강남」 검색 시 ' +
          (with역Normalized.data?.result?.totalCount ?? 0) +
          '건. ODsay 역명에는 「역」이 없습니다.',
      })
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err))
    } finally {
  ...
  const runPathTest = useCallback(async () => {
    setRouteLoading(true)
    setRouteError(null)
    setRouteResult(null)
    try {
      const data = await getOdsayPath(routeStart, routeEnd)
      setRouteResult(data)
    } catch (err) {
    ...
    <div className='mx-auto min-h-screen max-w-lg bg-slate-50 px-4 py-6 text-slate-800'>
      <header className='mb-6'>
        ...
        <p className='mt-2 text-sm text-slate-600'>
          ...
          <code className='rounded bg-white px-1'>laneName</code>으로 노선별
          구분.
        </p>
      </header>
      ...
      <section className='mb-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm'>
        <p className='mb-2 text-sm font-semibold text-slate-800'>
          길찾기 경로 테스트 (searchPubTransPathT)
        </p>
        <p className='mb-3 text-xs text-slate-500'>
          출발·도착역을 넣으면{' '}
          <code className='rounded bg-slate-100 px-1'>/odsay/path</code>로
          경로·소요시간을 조회합니다. (「역」은 자동 제거)
        </p>
        <div className='flex flex-col gap-2 sm:flex-row'>
          <input
            ...
            className='flex-1 rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-slate-400'
            placeholder='출발역 (예: 연신내)'
          />
          <input
            value={routeEnd}
            onChange={(e) => setRouteEnd(e.target.value)}
            className='flex-1 rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-slate-400'
            placeholder='도착역 (예: 봉은사)'
          />
          <button
            type='button'
            onClick={runPathTest}
            ...
            className='rounded-lg bg-slate-800 px-4 py-2 text-sm font-medium text-white disabled:opacity-40'
          >
            {routeLoading ? '조회 중…' : '경로 조회'}
          </button>
        </div>

        {routeError && (
          <div className='mt-3 rounded-lg border border-rose-200 bg-rose-50 p-3 text-sm text-rose-800'>
            <p className='font-semibold'>경로 오류</p>
            <p className='mt-1 whitespace-pre-wrap'>{routeError}</p>
          </div>
        )}
        ...
        {routeResult && (
          <div className='mt-3 space-y-2'>
            <p className='text-xs text-slate-500'>
              {routeResult.start} → {routeResult.end} · 경로{' '}
              {routeResult.path_count}개
            </p>
            {(routeResult.summaries ?? []).map((s, i) => (
              <div
                key={i}
              ...
              >
                <div className='flex flex-wrap items-center gap-x-3 gap-y-1'>
                  <span className='font-semibold text-slate-800'>
                    총 {s.total_time_min}분
                  </span>
                  <span>환승 {s.transfer_count ?? 0}회</span>
                  {s.payment != null && (
                    <span>{s.payment.toLocaleString()}원</span>
                  )}
                </div>
                {s.lines?.length > 0 && (
      ...
      {result && (
        <section className='space-y-4'>
          <div className='rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900'>
            ...
            <p className='mt-1'>
              입력 「{result.query.input}」 → API 「
              {result.query.apiStationName}」 ·{' '}
              {result.data?.result?.totalCount ?? stations.length}건
            </p>
            {result.query.stripped역 && (
              <p className='mt-1 text-xs text-emerald-800'>
                ODsay는 역명에 「역」을 붙이지 않습니다. 접미사를 제거한 뒤
                검색했습니다.
              </p>
            )}
          </div>
          ...
          {stations.length === 0 ? (
            <div className='rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-600'>
              검색 결과 없음. 「역」 접미사 제거·2자 이상·서울 필터를
              확인하세요.
            </div>
          ) : (
                    ...
                    {items.map((s) => (
                      ...
                      >
                        <p className='font-medium'>
                          {formatOdsayStationLabel(s)}
                        </p>
                        <p className='mt-0.5 text-slate-500'>
                          stationID {s.stationID} · arsID {s.arsID ?? '-'} ·
                          type {s.type ?? '-'}
                        </p>
                      </li>
                    ))}
