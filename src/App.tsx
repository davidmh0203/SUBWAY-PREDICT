...
import type { AppView, RoutePath, SearchForm } from "@/lib/types";

const VIEWS: AppView[] = ["home", "results", "detail", "macro"];
...
function readHashView(): AppView | null {
  const hash = window.location.hash.replace("#", "");
  return VIEWS.includes(hash as AppView) ? (hash as AppView) : null;
}

export default function App() {
  const [view, setView] = useState<AppView>(() => readHashView() ?? "home");
  ...
  const handleTimeChange = useCallback((targetTime: Date) => {
  ...
  const bottomNav = useMemo(
    () => [
      ...
      { id: "results" as const, label: "경로", icon: Route },
      { id: "detail" as const, label: "상세", icon: Train, disabled: !selectedRoute },
      { id: "macro" as const, label: "노선도", icon: Map },
    ],
    [selectedRoute],
  );
